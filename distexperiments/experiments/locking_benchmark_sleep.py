...
parameter_grid = ParameterGrid(
    padding = [8],
    worker=[1,2,4,8,16,32,64,128,256],
    sleep=[0,8,16,32,64],
    locks=[200000],
...
def locking_benchmark(servers, padding, worker,sleep,locks, options):    
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0 sudo ip netns exec ib0  ./locking_benchmark -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="sleep_locking_benchmark.csv" -run_for_seconds=30 -padding={padding} -tag={worker} -nopinThreads -lock_count={locks} {options} -sleep={sleep}'
