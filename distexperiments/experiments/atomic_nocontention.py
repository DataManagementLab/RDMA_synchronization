...
parameter_grid = ParameterGrid(
    alignment=[8,16,32,64,128,256,512,1024,2048,4096,8192],
    padding = [0,8],
...
def alignment(servers, alignment, padding, worker):    
    servers.cd("/home/tziegler/rdma_synchronization/build/frontend")        
    cmds = []
    cmd = f'numactl --membind=0 --cpunodebind=0 sudo ip netns exec ib0 ./atomic_alignment -ownIp={servers[0].ibIp} -storage_node -worker={worker}'
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0  sudo ip netns exec ib0 ./atomic_alignment -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="atomic_nocontention.csv" -run_for_seconds=30 -alignment={alignment} -padding={padding} -tag={worker} -nopinThreads -compute_id={i-1}'
