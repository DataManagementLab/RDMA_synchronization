...
def fa_benchmark(servers, worker):    
    ...
    cmds = []
    cmd = f'numactl --membind=0 --cpunodebind=0 sudo ip netns exec ib0 ./read_benchmark -ownIp={servers[0].ibIp} -storage_node -worker={worker}'
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0  sudo ip netns exec ib0 ./read_benchmark -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="read_baseline_benchmark.csv" -run_for_seconds=30 -tag={worker} -nopinThreads -compute_id={i}'
