...
def alignment(servers, alignment, padding, worker):    
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0  sudo ip netns exec ib0 ./atomic_alignment -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="atomic_nocontention_hypo.csv" -run_for_seconds=30 -alignment={alignment} -padding={padding} -tag={worker} -nopinThreads -compute_id={i-1}'
