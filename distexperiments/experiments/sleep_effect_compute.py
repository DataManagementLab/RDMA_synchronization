...
parameter_grid = ParameterGrid(
    storageNodes = [1],
    padding = [8],
    worker=[1,2,4,8,16,32,64,128,256,512,1024],
    sleep=[0,8,16,32,64],
    locks=[200000],
...
def locking_benchmark(servers, storageNodes, padding, worker,sleep,locks, options):    
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0 sudo ip netns exec ib0  ./pause_effect -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="effect_compute_two.csv" -run_for_seconds=30 -padding={padding} -tag={worker} -nopinThreads -lock_count={locks} {options} -sleep={sleep} -storage_nodes={storageNodes}'
