...
def batch_benchmark(servers, padding,storageNodes, worker, batch,locks):    
    ...
    cmds = []
    cmd = f'numactl --membind=0 --cpunodebind=0 sudo ip netns exec ib0 ./batched_reads -ownIp={servers[0].ibIp} -storage_node -worker={worker} -lock_count={locks} -padding={padding} -dramGB=10 -storage_nodes={storageNodes}'
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0  sudo ip netns exec ib0 ./batched_reads -ownIp={servers[i].ibIp} -worker={work} -all_worker={worker} -csvFile="batch_benchmark.csv" -run_for_seconds=30 -padding={padding} -tag={worker} -nopinThreads -lock_count={locks} -storage_nodes={storageNodes} -batch={batch}'
