...
def nam_benchmark(servers, padding, worker,locks, options):    
    ...
    for i in range(0, NUMBER_NODES):
        cmd = f'numactl --membind=0 sudo ip netns exec ib0  ./nam_experiment -ownIp={servers[i].ibIp} -worker={worker} -csvFile="nam_db_benchmark_restart.csv" -run_for_seconds=30 -padding={padding} -tag={tag} -lock_count={locks} -storage_nodes={NUMBER_NODES} -storage_node {options} -dramGB=10'
