...
parameter_grid = ParameterGrid(
    ...
    locks=[200000],
    # options=["","-speculative_read","-speculative_read -write_combining", "-speculative_read -write_combining -order_release"],
...
def locking_benchmark(servers, padding, worker,locks, options):    
    servers.cd("/home/tziegler/rdma_synchronization/build/frontend")        
    cmds = []
    cmd = f'numactl --membind=0 --cpunodebind=0 sudo ip netns exec ib0  ./locking_benchmark -ownIp={servers[0].ibIp} -storage_node -worker={worker} -lock_count={locks} -padding={padding} -dramGB=10'
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0 sudo ip netns exec ib0  ./locking_benchmark -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="locking_benchmark.csv" -run_for_seconds=30 -padding={padding} -tag={worker} -nopinThreads -lock_count={locks} {options}'
