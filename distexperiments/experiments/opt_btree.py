...
def locking_benchmark(servers, padding, worker,options):    
    ...
    cmds = []
    cmd = f'numactl --membind=0 --cpunodebind=0 sudo ip netns exec ib0  ./opt_btree -ownIp={servers[0].ibIp} -storage_node -worker={worker} -padding={padding} -dramGB=10'
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0 sudo ip netns exec ib0  ./opt_btree -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="btree_benchmark.csv" -run_for_seconds=30 -padding={padding} -tag={worker} -nopinThreads {options}'
