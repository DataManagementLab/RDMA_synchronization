...
def fa_benchmark(servers, worker, options):    
    servers.cd("/home/tziegler/rdma_synchronization/build/frontend")        
    cmds = []
    cmd = f'numactl --membind=0 --cpunodebind=0 sudo ip netns exec ib0 ./cas_benchmark -ownIp={servers[0].ibIp} -storage_node -worker={worker}'
    ...
    for i in range(1, numberNodes+1):
        cmd = f'numactl --membind=0  sudo ip netns exec ib0 ./cas_benchmark -ownIp={servers[i].ibIp} -all_worker={worker} -worker={work} -csvFile="atomic_contention_benchmark.csv" -run_for_seconds=30 -tag={worker} -nopinThreads {options}'
