...
int main(int argc, char* argv[]) {
   ...
   if (FLAGS_storage_node) {
      ...
      benchmark::ALIGNMENT_workloadInfo experimentInfo{benchmark, FLAGS_alignment, FLAGS_padding};
      ...
      for (uint64_t t_i = 0; t_i < FLAGS_worker; ++t_i) {
         compute.getWorkerPool().scheduleJobAsync(t_i, [&, t_i]() {
            ...
            // auto addr = desc.start + 64 +  (thread_id * FLAGS_alignment) + (FLAGS_padding * thread_id);
            auto addr = desc.start + 64 +  ((thread_id * FLAGS_alignment) % 64) + (FLAGS_padding * thread_id);           
            ...
            rdma_barrier_wait(desc.start,1,barrier_buffer, *rctx );
            uint64_t desired = 0;
            uint64_t expected = 0;
            while (keep_running) {
               ...
               rdma::postCompareSwap(expected, desired, old, *(rctx),rdma::completion::signaled, addr);
               int comp{0};
               ibv_wc wcReturn;
               while (comp == 0) {
                  _mm_pause();
      ...
      if (FLAGS_record_latency) {
         ...
         for (uint64_t t_i = 0; t_i < FLAGS_worker; ++t_i) {
            compute.getWorkerPool().scheduleJobAsync(t_i, [&, t_i]() {
               ...
               auto desc = threads::Worker::my().catalog[0];
               uint64_t thread_id = (FLAGS_compute_id * FLAGS_worker) + t_i;
               auto addr = desc.start + (thread_id * FLAGS_alignment) + (FLAGS_padding * thread_id);
               uint64_t ops = 0;
               auto* tl_rdma_buffer = static_cast<uint64_t*>(cm.getGlobalBuffer().allocate(64, 64));
               uint64_t expected = 0;
               uint64_t desired = 0;
...

         {
            ...
            if (!csv_initialized) { latency_file << "workload,worker,tag,alignment,padding,latency" << std::endl; }
            for (uint64_t s_i = 0; s_i < 1000; s_i++) {
               latency_file << benchmark << "," << FLAGS_worker << "," << FLAGS_tag << "," << FLAGS_alignment << "," << FLAGS_padding << ","
                            << microsecond_latencies[s_i] << std::endl;
            }
            latency_file.close();
         }
         ...
         bool csv_initialized = std::filesystem::exists(filename);
         latency_file.open(filename, open_flags);
         if (!csv_initialized) { latency_file << "workload,workers,tag,alignment,padding,min,median,max,95th,99th,999th" << std::endl; }
         latency_file << benchmark << "," << FLAGS_worker << "," << FLAGS_tag << ","  << FLAGS_alignment << "," << FLAGS_padding << ","
                      << (microsecond_latencies[0]) << "," << (microsecond_latencies[microsecond_latencies.size() / 2]) << ","
                      << (microsecond_latencies.back()) << "," << (microsecond_latencies[(int)(microsecond_latencies.size() * 0.95)]) << ","
